...
function genFetchUri(id) {
  return encodeURI(`http://localhost:7777/api/v1/loops?query={"_id"
  :"${id}"}&populate=["episode", "series"]`);
}
...
const actions = {
  ...
  async fetchLoop({ commit }, id) {
    try {
      commit('SET_LOADING', true);
      const response = await fetchGen(genFetchUri(id), fetchOptions);
